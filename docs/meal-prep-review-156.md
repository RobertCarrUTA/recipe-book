...
The numerical inputs used for this review are preserved in the [PR's historical calculation record](https://github.com/RobertCarrUTA/recipe-book/blob/394f5ed6e4478d62528a156c9896909caddff362/tests/fixtures/meal_prep_156_nutrition.json), not in the current test suite. Each profile records nutrients per 100 g, its source and any explicit proxy. Each recipe record lists full-batch edible grams, serving count, calculated values and rounded display values. These are review evidence, not a second editable recipe source. Review the estimates when ingredient quantities or servings change; CI does not calculate or certify nutrition.

For each nutrient, sum `(ingredient grams / 100) * nutrient per 100 g`, then divide by base servings. Raw fish/meat weights are paired with raw profiles; grain weights are dry; legumes use approximately 500 g drained cooked contents for two 15-ounce cans. All measured cooking fat, sauce and toppings are counted even though some may remain on pans or bowls. Water has no meaningful energy contribution. Teaspoon oil is modeled as 4.5 g; tablespoon oil as 13.5 g. Fine salt is weighed, not assumed interchangeable with coarse kosher salt. Citrus juice is approximated at 1 g/mL and minor herb/spice spoon weights are disclosed in the historical calculation record.
...
Ingredient introductions, split oil/salt, sauce yields, raw/cooked weights, safe endpoints, time dependencies and shopping entries were manually reviewed. Shared application tests cover recipe schema, collection membership, grocery grouping and generated-catalog consistency. Recipe wording, tasting status and nutrition assumptions remain editorial review responsibilities. All five remain `not-tried` with `rating: null`.

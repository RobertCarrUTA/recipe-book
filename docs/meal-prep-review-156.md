...
Nutrition was recalculated after replacing tofu and making flavor changes. Chicken noodle water heats from the start while edamame cooks in a separate small saucepan; this avoids two sequential water-heating cycles. Sesame/scallion amounts now portion exactly to 3 g and 6 g per meal. Mustard is included in the salmon allergen note. Salsa has an approximate 180 g measured portion. The hot-berbere variation supplies the actual total paprika measurement instead of requiring addition while cooking. An explicit grocery-group rule keeps red wine vinegar out of the Wine category without changing red wine's grouping.
